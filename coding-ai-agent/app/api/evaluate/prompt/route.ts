/**
 * API endpoint for running Prompt evaluation
 * POST /api/evaluate/prompt - Start evaluation
 * GET /api/evaluate/prompt - Get latest results or history
 */

import { NextRequest, NextResponse } from 'next/server';
import fs from 'fs/promises';
import path from 'path';
import { PromptEvaluator } from '@/evaluation/prompt/prompt-evaluator';
import { promptTestDataset } from '@/evaluation/prompt/test-dataset';

const RESULTS_DIR = path.join(process.cwd(), 'evaluation', 'prompt', 'results');
...
export async function POST(req: NextRequest) {
  try {
      ...
      async start(controller) {
        try {
          ...
          controller.enqueue(encoder.encode(`data: ${JSON.stringify({ 
            type: 'start', 
            message: 'Starting prompt evaluation...', 
            totalTests: promptTestDataset.length 
          })}\n\n`));
    ...
    console.error('Error in /api/evaluate/prompt POST:', error);
...
/**
 * GET /api/evaluate/prompt
 * Get evaluation results (latest or history)
 */
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('Error in /api/evaluate/prompt GET:', error);
    return NextResponse.json(
